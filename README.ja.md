...
- **必ず単一マシンを常時起動すること**。ルームはメモリ上にあり、マシン間で共有**されません**。複数マシンが動いていたり（Fly はデフォルトで 2 台作ることがあります）、`auto_stop_machines` でマシンが切り替わると、再接続が別マシンに着地してルームが見つからず接続が切れます。初回デプロイ後に一度だけ `fly scale count 1 -a <app>` を実行し、`fly.toml` のとおり `auto_stop_machines = "off"` / `min_machines_running = 1` を保ちます。
- **ルームは再起動で消えます**。再デプロイやマシン再起動で進行中のルームは消えます（参加者は新しいルームを作り直して再開）。永続化層はありません。
