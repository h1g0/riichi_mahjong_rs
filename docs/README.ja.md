...
このプロジェクトへのコントリビューションを歓迎します。ビルド・テストの方法と本プロジェクトの規約は [CONTRIBUTING.md](../CONTRIBUTING.md)（英語）に、コードベースの全体像は [docs/architecture.md](./architecture.md)（英語）にまとまっています。変更をどこに入れるべきか探すときは、まずこの 2 つを参照してください。
