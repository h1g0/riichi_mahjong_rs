...
- 細かなルールを対局ごとに柔軟に設定可能。例：喰いタン、複数ロンの可否（頭ハネ）、喰い替え、各種途中流局、役満の包、ダブル役満、三人麻雀の北抜きやツモ損など
...
このプロジェクトへのコントリビューションを歓迎します。ビルド・テストの方法と本プロジェクトの規約は [コントリビューションガイド](./CONTRIBUTING.ja.md)（[英語版](../CONTRIBUTING.md)）に、コードベースの全体像は [アーキテクチャ](./architecture.ja.md)（[英語版](./architecture.md)）にまとまっています。変更をどこに入れるべきか探すときは、まずこの 2 つを参照してください。

あわせて、リーチ麻雀の用語を理解し、コード、コメント、ドキュメントで一貫した用語を使用するために、[ユビキタス言語集](./glossary.ja.md)を参照してください。[英語版](./glossary.md)も利用できます。

なお、Issue やコメントは日本語でも構いませんが、コミットメッセージ・プルリクエストの説明・コード中のコメントは英語で記述してください。
