...
役の表は機械可読な形式でも [`data/yaku.json`](../data/yaku.json) として公開して
います。こちらは第三の命名体系である mjai のローマ字ラベルも含みます。スキーマは
...
同じ対応表を `Kind` をキーとした機械可読な形式で
[`data/yaku.json`](../data/yaku.json) に置いています。**コード識別子**が空欄の役は
そちらには含まれません。
